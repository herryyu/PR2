...
\mkbaresec{本书导航}
本书的前两章介绍了Rust，并在我们移步至\boldredtxt{第三章}的基础数据类型前提供了一段简短的总览。有关所有权和引用的概念
位于第\boldredtxt{四、五}章节。我们建议按顺序通读本书前五章。
\mkpar{1}
第\boldredtxt{六}至第\boldredtxt{十}章涵盖了该门语言的基础知识：表达式（\boldredtxt{第六章}）、错误处理（\boldredtxt{第七章}）、程序包
和模块（\boldredtxt{第八章}）、结构体（\boldredtxt{第九章}）以及枚举和模式（\boldredtxt{第十章}）。对这些章进行跳跃式阅读没问题，但一
定别跳过错误处理这章，相信我们。
\mkpar{1}
\boldredtxt{第十一章}涵盖了特型和泛型，也是最后两个你需要了解的重要概念。特型类似于Java或C\#里的接口。同时它们作为一种主要方式，使得
Rust能够允许你将你的自定义类型整合进该语言本身。\boldredtxt{第十二章}展示了特型是如何支持运算符重载的，另外\boldredtxt{第十三章}涵盖了
更多实用特型。
\mkpar{1}
了解特型和泛型之后便可解锁本书的余下部分。第\boldredtxt{十四、十五}章涵盖了闭包和迭代器这两个你绝对
不想错过的强大工具。对于剩余章节你可以按自由顺序进行阅读，或者根据需求对它们进行深度挖掘。这些章节涵盖了该门语言的剩余部分：集合（\boldredtxt{第十六章}）、字符串和文本（\boldredtxt{第十七章}）、输
入输出（\boldredtxt{第十八章}）、并发（\boldredtxt{第十九章}）、异步编码（\boldredtxt{第二十章}）、宏（\boldredtxt{第二十一章}）、unsafe编码（\boldredtxt{第二十二章}）以及如何在其它语言中调用Rust函数（\boldredtxt{第二十三章}）。
...
\mkpar{0.5}
\itred{https://github.com/ProgrammingRust.}
\mkpar{0.5}
...
\mkpar{1}
我们不强制要求，但也欢迎你在进行引用时注明出处。出处通常包含书名、作者、出版商以及书籍ISBN号。例如：\textit{"Programmi
ng Rust,Second Edition by Jim Blandy, Jason Orendorff, and Leonora F.S. Tindall(O'Reilly). Copyright 2
021 JimBlandy, Leonora F.S. Tindall, and Jason Orendorff, 978-1-492-05259-3."}
\mkpar{1}
若你认为你对于示例代码的使用脱离了合理范畴，或是不属于上述的授权论列，请尽情通过邮箱联系我们：
\mkpar{0.5}
\itred{permissions@oreilly.com}
\mkpar{0.5}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\mkbaresec{O'Reilly在线学习}
...
频资源。对于更多信息，请访问\itred{http://oreilly.com}。
...
与本书有关的评论和问题，请发给出版社：
    \begin{quote}
        ...
        707-829-0104（传真）
    \end{quote}
欢迎访问本书主页，我们于此列出了勘误表、示例以及其它额外信息。请通过\itred{https://oreil.ly/programming-rust-2e}来访
问。
\mkpar{1}
通过发送电邮至\itred{bookquestions@oreilly.com}以提出关于本书的评论或者技术问题。
\mkpar{1}
访问\itred{http://www.oreilly.com}获得更多有关我们的书籍和课程的信息。
\mkpar{1}
在Facebook上找到我们: \itred{http://facebook.com/oreilly}
\mkpar{1}
在Twitter上关注我们: \itred{http://twitter.com/oreillymedia}
\mkpar{1}
在Youtube上关注我们: \itred{http://youtube.com/oreillymedia}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\mkbaresec{致谢}
你手里所持的本书很大程度上得益于我们官方技术审稿人的关注: Brain Anderson, Carol Nichols, 和Erik Nordin; 以及我们的
译者: Hidemoto Nakada（中田 秀基）（日本语），李松峰先生（简体中文），以及Adam Bochenek和Krzysztof Sawka（波兰语）。
\mkpar{1}
许多其它非官方审稿人阅读了本书初稿并提供了宝贵的反馈。我们想对于Eddy Bruel, Nick Fitzgerald, Graydon Hoare, Michael
 Kelly, Jeffrey Lim, Jakob Olesen, Gian-Carlo Pascutto, Larry Rabinowitz, Jaroslav Šnajdr, Joe Walker, 
